        ...
        case 'create-installment-payment':
            ...
            case 'create_installment_with_discount':
                if (!$permissions['can_create_installments']) {
                    jsonResponse(false, null, 'Você não tem permissão para criar mensalidades');
                    break;
                }
                
                try {
                    // Incluir gerenciador de desconto
                    if (!class_exists('InstallmentDiscountManager')) {
                        if (file_exists(__DIR__ . '/installment_discount_manager.php')) {
                            require_once __DIR__ . '/installment_discount_manager.php';
                        } else {
                            throw new Exception('Sistema de desconto não instalado. Execute a migração primeiro.');
                        }
                    }
                    ...
                    // Log para debug
                    error_log("=== CRIANDO MENSALIDADE COM DESCONTO ===");
                    error_log("Usuario: " . $usuario['email']);
                    error_log("Discount Data: " . json_encode($discountData));
                    
                    // Validações obrigatórias
                    $requiredFields = ['customer', 'billingType', 'description', 'dueDate'];
                    ...
                    if (!empty($discountData['enabled']) && $discountData['enabled'] === 'true') {
                        ...
                        if ($discountType === 'FIXED' && $discountValue >= $installmentValue) {
                            throw new Exception('Desconto fixo não pode ser maior que o valor da parcela');
                        }
                        
                        if ($discountType === 'PERCENTAGE' && $discountValue >= 100) {
                            throw new Exception('Desconto percentual não pode ser 100% ou mais');
                        }
                        ...
                        error_log("Desconto configurado: " . json_encode($processedDiscountData));
                    }
                    ...
                    if ($totalSavings > 0) {
                        ...
                        $successMessage .= "💰 <strong>COM DESCONTO:</strong> R$ " . number_format($valueWithDiscount, 2, ',', '.') . " por parcela<br>";
                        $successMessage .= "💸 <strong>ECONOMIA TOTAL:</strong> <span class='text-success fw-bold'>R$ " . number_format($totalSavings, 2, ',', '.') . "</span><br>";
                    }
                    ...
                    error_log("Mensalidade criada com sucesso - ID: {$result['data']['installment']}, Desconto: R$ " . number_format($totalSavings, 2));
            ...
            case 'get-installment-with-discount':
                try {
                    ...
                    // Incluir gerenciador se necessário
                    if (!class_exists('InstallmentDiscountManager')) {
                        require_once __DIR__ . '/installment_discount_manager.php';
                    }
                    ...
                    ], "Mensalidade carregada com informações de desconto");
            ...
            case 'discount-report':
                try {
                    ...
                    ], "Relatório de descontos gerado");
                    
                } catch (Exception $e) {
                    error_log("Erro no relatório de descontos: " . $e->getMessage());
                    jsonResponse(false, null, $e->getMessage());
                }
                break;
